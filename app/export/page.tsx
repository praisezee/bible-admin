		...
		const structuredBooks: BookData[] = books.map((book) => {
			...
			const structuredChapters: ChapterData[] = bookChapters.map((chapter) => {
				const chapterVerses = verses.filter(
					(verse) => verse.chapterId === chapter.id
				);
	...
	const handleExport = async () => {
		...
		try {
			...
			toast({
				...
				description: "Yahuah Dabar  data has been exported successfully",
			});
		} catch (error) {
			const errorMessage =
		...
		<DashboardLayout>
			<div className="flex items-center justify-between">
				<div>
					<h1 className="text-lg font-semibold md:text-2xl">
						Export Yahuah Dabar Data
					</h1>
					<p className="text-muted-foreground">
						Download complete Yahuah Dabar database as JSON
					</p>
				</div>
			</div>
			...
			<div className="grid gap-6 md:grid-cols-2">
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Export the complete Yahuah Dabar database including all books, chapters,
							and verses in JSON format.
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
						...
						<Button
							...
							{isExporting ? "Exporting..." : "Export Yahuah Dabar  Data"}
						</Button>
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Details about the exported data format and structure.
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
						{exportStats && (
						...
						<div className="space-y-3">
							...
							<div className="text-sm text-muted-foreground space-y-1">
								<p>• File name: Yahuah Dabar -export-YYYY-MM-DD.json</p>
								<p>• Format: UTF-8 encoded JSON</p>
								<p>• Structure: Compatible with Yahuah Dabar Data interface</p>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Preview of the JSON structure that will be exported.
					</CardDescription>
				</CardHeader>
				<CardContent>
